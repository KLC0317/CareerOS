...
import { Award, Layers, Target, CheckCircle2, Users, TrendingUp, Brain, BookOpen, Eye, EyeOff, Send, Calendar, Trophy, Archive, X } from 'lucide-react';
...
export default function SkillActivationMap() {
  ...
  // Auto-open sidebar if dashboard alert navigation set the flag
  useEffect(() => {
    const flag = sessionStorage.getItem('openCohortSidebar');
    if (flag === '1') {
      sessionStorage.removeItem('openCohortSidebar');
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <motion.div
            ...
            className={`kpi-card ${kpi.cardStyle} p-4 rounded-2xl border transition-all duration-200 flex flex-col justify-between gap-1.5 cursor-default`}
          >
      ...
      `}} />
        ...
        <div className={`transition-all duration-300 ${isSidebarOpen ? 'lg:w-[67%]' : 'w-full'} flex flex-col gap-4 p-5 rounded-2xl border border-slate-200/90 bg-white relative shadow-xs`}>
            ...
            <div className="graph-legend absolute top-3 right-3 flex flex-wrap items-center gap-3 text-[9px] px-2.5 py-1.5 rounded-lg border backdrop-blur-md z-20 shadow-xs">
              ...
              {selectedPeerId && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="graph-legend-label font-medium">Peer Target</span>
                </div>
              )}
              <div className="flex items-center gap-1">
                ...
                return (
                  ...
                  >
                    ...
                    <circle
                      ...
                      className={`transition-all duration-300 ${isInspected
                    ...
                    <text
                      ...
                      className={`text-[10px] font-extrabold font-mono tracking-tight transition-all duration-350 ${isInspected ? 'scale-110 font-black' : ''
                        }`}
                    >
                      R{dagNode?.rank}
                    </text>
                    ...
                    >
                      <div className="flex flex-col items-center">
                        <span
                          className={`text-[11px] font-extrabold text-center px-2 py-0.5 rounded-lg border shadow-sm transition-all duration-300 backdrop-blur-xs ${isInspected
          ...
          {selectedMapNode ? (
            ...
            <div className="graph-helper flex gap-2 items-center rounded-xl p-3.5 text-[11px] leading-normal">
        ...
        {isSidebarOpen && (
          <div id="cohort-network-section" className="lg:w-[33%] flex flex-col gap-4 p-5 rounded-2xl border border-slate-200 bg-slate-900/[0.02] backdrop-blur-md shadow-xs transition-all duration-300 relative z-20">
                      ...
                      {filteredPeers.slice(0, 15).map((peer) => {
                        ...
                        return (
                          <div
                            ...
                            className={`p-3 rounded-xl border text-xs transition-all duration-200 cursor-pointer shadow-[0_2px_6px_rgba(0,0,0,0.01)] ${isSelected
